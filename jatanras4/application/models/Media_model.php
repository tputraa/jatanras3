...
class Media_model extends CI_Model {	
   ...
   */
		...
		if($query->num_rows()) 
		{
			...
			{	
				...
				if(file_exists($file_path))
				{
					if($md->is_image) 
					{
						$img_url = $anchor_url = $media_path.'/'.$region.'/'.$username.$user_id.'/'.$path.$md->file_name;						
						...
						if (($md->image_width > $this->img_width) || ($md->image_height > $this->img_width))
						{
							...
							$url = $media_path.'/'.$region.'/'.$username.$user_id.'/'.$path.'thumb/'.$md->file_name;
					...
					else
					{
						...
						$data['files'][] = array(
								...
								'url'	  	  	=>	$media_path.'/'.$region.'/'.$username.$user_id.'/'.$path.$md->file_name,
	...
	/* Cari file */
	public function get_search_file($path = NULL)
	{	
		...
		if ($folderpath == "/") {
			$this->db->select('*');
			$this->db->like('raw_name', $keyword);
			$this->db->where('user_id', $user_id);
		}else {
			$this->db->select('*');
			$this->db->like('raw_name', $keyword);
			$this->db->like('file_path', '/'.$folderpath, 'after');
			$this->db->where('user_id', $user_id);
		}
			...
			{	
				...
				if(file_exists($file_path))
				{
					if($md->is_image) 
					{
						...
						$img_url = $anchor_url = $media_path.'/'.$region.'/'.$username.$user_id.'/'.$md->file_path.$md->file_name;						
					...
					else
					{
						...
						//echo $media_path.'/'.$region.'/'.$username.$user_id.'/'.$md->file_path.$md->file_name;
						...
						$data['files'][] = array(
								...
								'path'		  	=>  $path.$md->file_name,									
								//'url'	  	  	=>	$media_path.'/'.$region.'/'.$username.$user_id.'/'.$path.$md->file_name,
								'url'	  	  	=>	$media_path.'/'.$region.'/'.$username.$user_id.'/'.$md->file_path.$md->file_name,
	...
	public function get_data($id) {
		$sql ="SELECT m.id, m.user_id, m.raw_name, m.file_name,
				m.file_type, m.file_path, m.file_size,
				s.name
				FROM
				media m
				INNER JOIN users s ON m.user_id = s.id
				WHERE m.user_id='".$id."'";
    ...
    /**
	 ** add Rusmana insert into database, when copy file*
	 */
     
    public function newMediaFromCopy($param,$newPath){
        
        $q = "SELECT user_id,raw_name,file_name,file_type,file_path,file_ext,file_size,is_image,";
        $q.= "image_width,image_height,image_type FROM ".$this->table. " WHERE user_id='".$param['user_id']."' AND ";
        $q.= "file_name ='".$param['file_name']."' AND file_path ='".$param['file_path']."'";
             
        $res = $this->db->query($q);
        $res = $res->row();
               
        if ($res){
            $q = "INSERT INTO ".$this->table. " (user_id,raw_name,file_name,file_type,file_path,file_ext,";
            $q.= "file_size,is_image,image_width,image_height,image_type) value('".$res->user_id."','".$res->raw_name."',";
            $q.= "'".$res->file_name."','".$res->file_type."','".$newPath."','".$res->file_ext."','".$res->file_size."',";
            $q.= "'".$res->is_image."','".$res->image_width."','".$res->image_height."','".$res->image_type."')";
            $res = $this->db->query($q);
            if ($res){
                return 1;    
            }else{
    ...
    /**
	 ** add Rusmana delete in database, di gunakan di move*
	 */
    public function deleteMedia($param){
        $q = "DELETE FROM ".$this->table. " WHERE user_id='".$param['user_id']."' AND ";
        $q.= "file_name ='".$param['file_name']."' AND file_path ='".$param['file_path']."'";
