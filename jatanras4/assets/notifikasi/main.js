...
function notificationStream(id) {
	$.ajax({
		url: "/jatanrasdocumen/realtime/notification",
		dataType: 'json',
		data: {'user_id': id}
    }).done(function (data) {
...
function append_li(data) {
	...
	$.each(data, function(index, obj) {
		...
		'<li data-notif-id="' + obj.id + '">' +
			'<a href="#" class="notification-item"><span class="dot bg-danger"></span>' +
				obj.message +
			'</a>' +
		'</li>';
...
$('#notif_anchor').click(function(event) {
	...
	if (is_new_notif) {
		$.ajax({
			url: "/jatanrasdocumen/realtime/user/update_last_notif",
			dataType: 'json',
