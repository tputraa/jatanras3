...
class Dokumen_model extends CI_Model {
	...
	*/
	...
	{        
		...
		$this->db->select('pasal.id, pasal.kasus, media.id, users.name as nama, media.kanit_id, media.no_lp, media.nama_pelapor, media.tanggal_kejadian, media.pelaku, media.is_status'); 
		$this->db->join('pasal', 'media.pasal_id = pasal.id');
		$this->db->join('users', 'users.id = media.penyidik_id', 'LEFT');
	...
	{        
		$this->db->select('*'); 
		//$this->db->join('pasal', 'media.pasal_id = pasal.id');
		$this->db->where('media_detail.media_id',$id);
