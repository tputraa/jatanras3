...
class Penyidik extends MY_Controller {
	...
	public function __construct()
	{
		...
		$this->limit = $this->session->userdata('penyidik.filter.limit');	
	}
	...
	{	
		...
		$config = array();
		$config['base_url'] = base_url() . 'admin/penyidik/index';
		$config['total_rows'] = $this->penyidik_model->record_count();
		...
		$data['users'] = $this->penyidik_model->get_list($config['per_page'], $page);
	...
	public function get_results()
	{
		...
		redirect(site_url('admin/penyidik'),'refresh');
	}
	...
	public function print_users()
	{
        ...
        $users = $this->penyidik_model->get_data();
	...
	public function edit_details($id = NULL)
	{
		...
		if($id != NULL) 
		{
			$data['item'] 	= $this->penyidik_model->get_item($id);
			$data['kanit']	= $this->penyidik_model->get_datakanit();
		}

		$data['kanit']	= $this->penyidik_model->get_datakanit();
		$data['page'] 	= 'admin/edit_penyidik';		
		$this->load->view($this->view,$data);
	}
	...
	{		
		...
		$this->form_validation->set_rules('kanit_id', 'Nama Kanit', 'trim|required|strip_tags');
		...
		*/
		...
		{ 
			...
			$result = $this->penyidik_model->register();
		...
		{
			// update user details
			$result = $this->penyidik_model->update_details();
	...
	function delete ($id) {
		...
		$result = $this->db->delete('penyidik', $where);
		if ($result) {
			redirect(site_url('admin/penyidik'),'refresh');
		}
	...
	function create_user() {
		...
		$rows = $this->db->query("SELECT * FROM penyidik ORDER BY id DESC LIMIT 1")->row();
